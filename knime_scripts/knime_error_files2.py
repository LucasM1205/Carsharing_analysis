...
# Funktion zur Häufigkeitsanalyse für `vehicle_type_id`
def get_most_frequent_vehicle_type(neighbors):
    vehicle_types = []
        ...
        for station in neighbor.get("data", {}).get("stations", []):
            if "vehicle_types_available" in station:
                for vehicle in station["vehicle_types_available"]:
                    if isinstance(vehicle.get("vehicle_type_id"), str):
        ...
        if "vehicle_types_available" in station:
            for vehicle in station["vehicle_types_available"]:
                if "vehicle_type_id" not in vehicle or not isinstance(vehicle["vehicle_type_id"], str):
                    vehicle["vehicle_type_id"] = get_most_frequent_vehicle_type(neighbors)
